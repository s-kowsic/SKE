  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-5xl mx-auto bg-industrial-800 p-8 rounded-lg border border-industrial-700 mb-12">
        <h2 className="text-2xl font-bold mb-6">{editingId ? t('admin.editProduct') : t('admin.addProduct')}</h2>
        <form onSubmit={handleAddProduct} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-gray-400 mb-1 text-sm">{t('admin.productName')}</label>
            ...
            <div>
              <label className="block text-gray-400 mb-1 text-sm">{t('admin.categoryType')}</label>
              <input required type="text" list="category-options" className="input-field" value={productForm.type} onChange={e => setProductForm({...productForm, type: e.target.value})} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-gray-400 mb-1 text-sm">{t('admin.sizeDimensions')}</label>
            ...
            <div>
              <label className="block text-gray-400 mb-1 text-sm">{t('admin.price')}</label>
            ...
            <div>
              <label className="block text-gray-400 mb-1 text-sm">{t('admin.initialStock')}</label>
          ...
          <div>
            <label className="block text-gray-400 mb-1 text-sm">{t('admin.productImage')}</label>
            ...
            {uploading && <p className="text-industrial-orange mt-2 text-sm">{t('admin.uploading')}</p>}
            {productForm.imageUrl && <p className="text-green-500 mt-2 text-sm truncate">{t('admin.imageSelected')}: {productForm.imageUrl}</p>}
          </div>
          <div>
            <label className="block text-gray-400 mb-1 text-sm">{t('admin.description')}</label>
          ...
          <div className="flex gap-4">
            <button type="submit" className="flex-1 btn-primary py-3">
              {editingId ? t('admin.updateProduct') : t('admin.publishProduct')}
            </button>
            {editingId && (
      ...
      <div className="max-w-5xl mx-auto bg-industrial-800 rounded-lg border border-industrial-700 overflow-hidden">
        <div className="p-6 border-b border-industrial-700">
          <h3 className="text-xl font-bold">{t('admin.currentCatalog')}</h3>
        </div>
            ...
            <tbody>
              {products.map(product => (
                <tr key={product._id} className="border-b border-industrial-700 hover:bg-industrial-700/50 transition-colors">
                  <td className="p-4">
                    {product.imageUrl ? (
                      <img src={getImageUrl(product.imageUrl)} alt={product.name} className="w-12 h-12 object-cover rounded" />
                    ) : (
                      <div className="w-12 h-12 bg-gray-700 rounded flex items-center justify-center text-xs">{t('admin.noImg')}</div>
                    )}
                  </td>
              ...
              {products.length === 0 && (
                <tr>
                  <td colSpan="6" className="p-8 text-center text-gray-500">{t('admin.noProducts')}</td>
                </tr>
              )}
            </tbody>
          </table>
