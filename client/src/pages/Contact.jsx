  ...
  return (
    <div className="min-h-screen bg-industrial-900 pt-20">
      ...
      <section className="py-20">
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16">
            ...
            >
              <h2 className="text-3xl font-bold text-white mb-2">{t('contact.requestQuote')}</h2>
              <p className="text-gray-400 mb-8">{t('contact.requestQuoteDesc')}</p>

              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-bold text-gray-400 mb-2">{t('contact.fullName')} *</label>
                    <input
                      type="text" required
                      className="input-field" placeholder={t('contact.name')}
                      value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-400 mb-2">{t('contact.emailAddress')} *</label>
                    <input
                      type="email" required
                      className="input-field" placeholder={t('contact.email')}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-bold text-gray-400 mb-2">{t('contact.phoneNumber')}</label>
                    <input
                      type="tel"
                      className="input-field" placeholder={t('contact.phoneNumber')}
                      value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-400 mb-2">{t('contact.estimatedQty')}</label>
                    <input
                      type="number" min="1"
                ...
                <div className="relative">
                  <label className="block text-sm font-bold text-gray-400 mb-2">{t('contact.productReq')} *</label>
                  <textarea
                    required rows="4"
                    className="input-field resize-none"
                    placeholder={t('contact.productReqPlaceholder')}
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <span className="text-sm text-gray-400 block mb-2">
                            {isSuggesting ? t('contact.aiAnalyzing') : t('contact.aiSuggestion')}
                          </span>
                          {!isSuggesting && suggestions.length > 0 && (
                ...
                >
                  {isSubmitting ? <><Loader2 className="animate-spin" size={20} /> {t('contact.submitting')}</> : t('contact.submitEnquiry')}
                </button>
              </form>
            </motion.div>
              ...
              <div className="bg-industrial-800 p-8 rounded-2xl border border-industrial-700 shadow-xl">
                ...
                <div className="space-y-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-white">{t('contact.headquarters')}</h4>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-white">{t('contact.directLine')}</h4>
                      <p className="text-gray-400">+91 6380657464</p>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-white">{t('contact.emailAddr')}</h4>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-white">{t('contact.workingHours')}</h4>
                      <p className="text-gray-400">{t('contact.mfg247')}</p>
                      <p className="text-gray-400">{t('contact.salesDesk')}</p>
                    </div>
                  </div>
                </div>
      ...
      <section className="py-20 bg-industrial-800 border-y border-industrial-700">
        <div className="container mx-auto px-6 max-w-4xl">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-white mb-4">{t('contact.faqTitle')}</h2>
            <p className="text-gray-400">{t('contact.faqDesc')}</p>
          </div>
      ...
      <section className="py-24 text-center">
        <div className="container mx-auto px-6">
          <h2 className="text-4xl font-bold text-white mb-6">{t('contact.urgentSupport')}</h2>
          <p className="text-xl text-gray-400 mb-10 max-w-2xl mx-auto">{t('contact.urgentDesc')}</p>
          <a href="tel:+919876543210" className="inline-flex items-center gap-3 bg-red-600 hover:bg-red-700 text-white py-4 px-10 rounded text-xl font-bold transition-all hover:scale-105 shadow-[0_0_20px_rgba(220,38,38,0.4)]">
            <Phone size={24} /> {t('contact.emergencyCall')}
          </a>
        </div>
      </section>
