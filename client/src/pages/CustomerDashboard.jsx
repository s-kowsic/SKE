...
export default function CustomerDashboard() {
  ...
  if (loading) return <div className="p-8 text-center text-xl text-gray-400">{t('dashboard.loadingProfile')}</div>;

  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="bg-industrial-800 p-8 rounded-lg border border-industrial-700 mb-8">
        <h2 className="text-3xl font-bold mb-2">{t('dashboard.welcome')}, {user?.name}</h2>
        <p className="text-gray-400">Email: {user?.email}</p>
      </div>

      <h3 className="text-2xl font-bold mb-6">{t('dashboard.orderHistory')}</h3>
      {orders.length === 0 ? (
            ...
            <div key={order._id} className="bg-industrial-800 p-6 rounded-lg border border-industrial-700 flex flex-col md:flex-row justify-between md:items-center">
              <div className="mb-4 md:mb-0">
                ...
                <p className="text-sm text-gray-400 mt-2">
                  {t('dashboard.orderItems')}: {order.products.map(p => p.productId?.name).join(', ')}
                </p>
              </div>
